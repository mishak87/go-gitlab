...
func TestUsersService_CreateKeyForUser(t *testing.T) {
	setup()
	defer teardown()

	input := &Key{Key: String("k"), Title: String("t")}

	mux.HandleFunc("/users/1/keys", func(w http.ResponseWriter, r *http.Request) {
		v := new(Key)
		json.NewDecoder(r.Body).Decode(v)
	...
	key, _, err := client.Users.CreateKeyForUser(1, input)
	if err != nil {
		t.Errorf("Users.GetKey returned error: %v", err)
	}
	...
	want := &Key{ID: Int(1)}
	if !reflect.DeepEqual(key, want) {
		t.Errorf("Users.CreateKeyForUser returned %+v, want %+v", key, want)
	}
}
...
func TestUsersService_DeleteKeyForUser(t *testing.T) {
	setup()
	defer teardown()

	mux.HandleFunc("/users/1/keys/1", func(w http.ResponseWriter, r *http.Request) {
		testMethod(t, r, "DELETE")
	})

	_, err := client.Users.DeleteKeyForUser(1, 1)
	if err != nil {
		t.Errorf("Users.DeleteKeyForUser returned error: %v", err)
	}
}
